...
describe("buildVaultLinkIndex", () => {
  it("resolves a wikilink target by filename stem, frontmatter id, title, and alias", () => {
    vault.writeText(
      "memories/anna-piano-1234abcd.md",
      memoryDoc({
        id: "mem_1234abcd",
    ...
    expect(index.resolve("anna-piano-1234abcd")).toBe("memories/anna-piano-1234abcd.md");
    expect(index.resolve("mem_1234abcd")).toBe("memories/anna-piano-1234abcd.md");
    expect(index.resolve("Anna — Piano Teacher")).toBe("memories/anna-piano-1234abcd.md");
    expect(index.resolve("Anna")).toBe("memories/anna-piano-1234abcd.md");
  ...
  it("returns the paths that wikilink to a file as its backlinks", () => {
    vault.writeText(
      "memories/anna-1.md",
      memoryDoc({ id: "mem_1", title: "Anna", body: "Plays piano." }),
    );
    vault.writeText(
      "memories/lessons-2.md",
      memoryDoc({ id: "mem_2", title: "Lessons", body: "Weekly with [[Anna]] at home." }),
    );
    vault.writeText(
    ...
    const backlinks = buildVaultLinkIndex(vault).backlinks("memories/anna-1.md");
  ...
  it("a self-link is not a backlink, and alias/heading link forms still count", () => {
    vault.writeText(
      "memories/anna-1.md",
      memoryDoc({ id: "mem_1", title: "Anna", body: "Recursive [[Anna]] mention." }),
    );
    vault.writeText(
      "memories/notes-2.md",
      memoryDoc({ id: "mem_2", title: "Notes", body: "See [[Anna#Schedule|her schedule]]." }),
    );

    const index = buildVaultLinkIndex(vault);
    expect(index.backlinks("memories/anna-1.md")).toEqual(["memories/notes-2.md"]);
  });

  it("lists a file's outbound links with their resolution (null = dangling)", () => {
    vault.writeText(
      "memories/anna-1.md",
      memoryDoc({ id: "mem_1", title: "Anna", body: "Linked from elsewhere." }),
    );
    vault.writeText(
      "memories/notes-2.md",
      memoryDoc({ id: "mem_2", title: "Notes", body: "[[Anna]] and [[Ghost Doc]]." }),
    );
  ...
  it("a file with malformed frontmatter is still addressable by its stem", () => {
    ...
    vault.writeText(
      "memories/anna-1.md",
      memoryDoc({ id: "mem_1", title: "Anna", body: "Target." }),
    );

    const index = buildVaultLinkIndex(vault);
    expect(index.resolve("broken")).toBe("references/broken.md");
    expect(index.backlinks("memories/anna-1.md")).toEqual(["references/broken.md"]);
  });

  it("honours the include filter (hidden files neither resolve nor backlink)", () => {
    vault.writeText("inbox/item-1.md", "Links [[Anna]].\n");
    vault.writeText(
      "memories/anna-1.md",
      memoryDoc({ id: "mem_1", title: "Anna", body: "Target." }),
    );
    ...
    expect(index.backlinks("memories/anna-1.md")).toEqual([]);
  });
});
